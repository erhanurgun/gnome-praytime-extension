...
// Test 1: Constructor ve getter'lar (yeni id parametresi dahil)
console.log('1. Constructor ve Getter Testleri:');
const testDate = new Date(2026, 0, 16, 5, 30, 0);
const prayer = new PrayerTime('imsak', 'İmsak', 'Imsak', testDate);

assertEqual(prayer.id, 'imsak', 'id getter doğru çalışır');
assertEqual(prayer.name, 'İmsak', 'name getter doğru çalışır');
assertEqual(prayer.nameEn, 'Imsak', 'nameEn getter doğru çalışır');
...
const noonPrayer = new PrayerTime('ogle', 'Öğle', 'Dhuhr', new Date(2026, 0, 16, 12, 45, 0));
assertEqual(noonPrayer.timeString, '12:45', 'timeString doğru formatlanır (12:45)');

const midnightPrayer = new PrayerTime('test', 'Test', 'Test', new Date(2026, 0, 16, 0, 5, 0));
...
const prayerAt10 = new PrayerTime('ogle', 'Öğle', 'Dhuhr', new Date(2026, 0, 16, 10, 0, 0));
...
const targetPrayer = new PrayerTime('ogle', 'Öğle', 'Dhuhr', targetTime);
...
const prayerFromString = new PrayerTime('ikindi', 'İkindi', 'Asr', isoString);
...
assertEqual(prayerFromString.id, 'ikindi', 'id string constructor\'da doğru');

// Test 6: Farklı id değerleri
console.log('\n6. Farklı ID Değerleri:');
const tahajjud = new PrayerTime('tahajjud', 'Teheccüd', 'Tahajjud', new Date(2026, 0, 16, 3, 30, 0));
assertEqual(tahajjud.id, 'tahajjud', 'Teheccüd id doğru');

const sahur = new PrayerTime('sahur', 'Sahur', 'Suhur', new Date(2026, 0, 16, 5, 0, 0));
assertEqual(sahur.id, 'sahur', 'Sahur id doğru');
